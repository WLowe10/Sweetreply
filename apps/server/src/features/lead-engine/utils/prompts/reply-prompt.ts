...
export const replyPrompt = ({ project, lead, characterLimit }: ReplyPromptData) => {
	...
	return {
		...
		system: `Reply to a social media post with a helpful response relevant to the poster's query or situation. Integrate a subtle mention of the specified dynamic product in a natural and non-affiliated manner, without expanding the bounds of the product description. Ensure the reply sounds human, maintaining a formal and concise tone without resembling a sales pitch. The response should be less than ${characterLimit} characters.`,
